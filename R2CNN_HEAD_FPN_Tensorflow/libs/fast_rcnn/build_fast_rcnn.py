...
class FastRCNN(object):
    ...
    def fast_rcnn_net(self):
        ...
        with tf.variable_scope('fast_rcnn_net'):
            with slim.arg_scope([slim.fully_connected], weights_regularizer=slim.l2_regularizer(self.weight_decay)):
                ...
                fast_rcnn_encode_boxes = slim.fully_connected(net, 4, activation_fn=None,
                                                              scope='regressor')
        ...
        with tf.variable_scope('fast_rcnn_net_rotate'):
            with slim.arg_scope([slim.fully_connected], weights_regularizer=slim.l2_regularizer(self.weight_decay)):
                ...
                #                                                scope='head_quadrant')
                # class-agnostic regression
                fast_rcnn_encode_boxes_rotate = slim.fully_connected(net_rotate, 5,
                                                                     activation_fn=None,
                                                                     scope='regressor')
                fast_rcnn_head_quadrant = slim.fully_connected(net_rotate, 4,
                                                               activation_fn=None,
                                                               scope='head_quadrant')
    ...
    def fast_rcnn_loss(self):
        with tf.variable_scope('fast_rcnn_loss'):
            ...
            with tf.variable_scope('fast_rcnn_classification_loss'):
                ...
                fast_rcnn_classification_loss = losses.focal_loss(prediction_tensor=minibatch_scores, target_tensor=minibatch_label_one_hot)
            ...
            with tf.variable_scope('fast_rcnn_classification_rotate_loss'):
                ...
                #                                                                          onehot_labels=minibatch_label_one_hot)
                fast_rcnn_classification_rotate_loss = losses.focal_loss(prediction_tensor=minibatch_scores_rotate, target_tensor=minibatch_label_one_hot)
            ...
            with tf.variable_scope('fast_rcnn_head_quadrant_loss'):
                ...
                fast_rcnn_head_quadrant_loss = losses.l1_smooth_losses(predict_boxes=minibatch_head_quadrant,
                                                                       ...
                                                                       object_weights=minibatch_object_mask,
                                                                       classes_weights=None)
                slim.losses.add_loss(fast_rcnn_head_quadrant_loss * 10) # More importance by the bigger weight
    ...
    def fast_rcnn_proposals(self, decode_boxes, scores):
        ...
        with tf.variable_scope('fast_rcnn_proposals'):
            ...
            scores, _ = tf.nn.top_k(scores[:, 1:], 1) # select the max score in all classes as nms score
            scores = tf.squeeze(scores)
            valid_indices = boxes_utils.nms_boxes(decode_boxes, scores,
                                                        iou_threshold=self.fast_rcnn_nms_iou_threshold,
                                                        max_output_size=self.fast_rcnn_nms_max_boxes_per_class,
                                                        name='second_stage_NMS')
    ...
    def fast_rcnn_proposals_rotate(self, decode_boxes, scores, head_quadrant):
        ...
        with tf.variable_scope('fast_rcnn_proposals'):
            ...
            scores, _ = tf.nn.top_k(scores[:, 1:], 1) # select the max score in all classes as nms score
            scores = tf.squeeze(scores)
            valid_indices = nms_rotate.nms_rotate(decode_boxes=decode_boxes,
                                                  scores=scores,
                                                  iou_threshold=self.fast_rcnn_nms_iou_threshold,
                                                  max_output_size=self.fast_rcnn_nms_max_boxes_per_class,
                                                  use_angle_condition=False,
