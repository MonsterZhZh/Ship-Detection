...
def read_and_prepocess_single_img(filename_queue, shortside_len, is_training):
    ...
    if is_training:
        img, gtboxes_and_label = image_preprocess.rotate_point(img_tensor=img, gtboxes_and_label=gtboxes_and_label)
...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    if dataset_name not in ['UAV', 'shapan', 'airplane', 'SHIP', 'HRSC2016', 'spacenet', 'pascal', 'coco']:
