...
def train():
    with tf.Graph().as_default():
        ...
        # Class-specific regression lr
        # lr = tf.train.piecewise_constant(global_step,boundaries=[np.int64(20000), np.int64(40000)],values=[0.0001, 0.00001, 0.000001])
        # Class-agnostic regression lr since fast_rcnn_loc_loss and fast_rcnn_loc_rotate_loss are too small, which caues 
        # unnormal gradient behavior
        lr = tf.train.piecewise_constant(global_step,boundaries=[np.int64(20000), np.int64(40000)],values=[0.000001, 0.0000001, 0.00000001])
