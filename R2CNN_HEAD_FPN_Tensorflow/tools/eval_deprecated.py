...
def eval_ship(img_num, mode):
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            fw2 = open('predict_horizontal_dict.pkl', 'wb')
...
def get_single_label_dict(predict_dict, gtboxes_dict, label):
    ...
    for i in range(len(rbox_images)):
        ...
        print('rbox_image:', rbox_image)
        for pre_box in predict_dict[rbox_image]:
            if pre_box['name'] == label and len(pre_box['bbox']) != 0:
                print('pre_box name:', pre_box['name'])
                print('pre_box bbox', pre_box['bbox'])
        ...
        if rbox_image in rboxes and rbox_image in gboxes:
            print('rboxes[rbox_image]:', len(rboxes[rbox_image]))
            print('gboxes[rbox_image]:', len(gboxes[rbox_image]))
...
if __name__ == '__main__':
    ...
    for label in NAME_LABEL_MAP.keys():
        ...
        print('rec:{}prec:{}ap:{}box_num:{}'.format(rec, prec, ap, box_num))
        rec1, prec1, ap1, box_num1 = eval(rboxes1, gboxes1, 0.5, False, mode=mode)
        print('rec1:{}prec1:{}ap1:{}box_num1:{}'.format(rec1, prec1, ap1, box_num1))
